...
## Pending server work (not yet built — for the next swagger + server prompt)

These are decided but NOT yet in swagger.yaml or implemented. Do not invent
alternatives; implement these shapes when the work is scheduled.

- **Account CRUD** (writes the new account columns: name, ui_lang,
  learn_base_lang, learn_target_lang, status):
  - `GET /me` — read account
  - `PATCH /me` — update name, ui_lang, learning pair
  - `DELETE /me` — delete account
  - `init_account` (post-registration) sets name + learning pair via `PATCH /me`.
- **Language pairs from config, not DB**: `GET /language-pairs` returns supported
  (base, target) pairs read from `application.yml`. This is a TEMPORARY stand-in
  for coverage-derived pairs (the real constraint is translation coverage in the
  data); fine for MVP, document as such. Do not add a pairs table.
- **Password reset / email confirmation** (auth surface grew):
  - `POST /auth/reset/request` — send a 4-digit code. Code lives in CACHE (e.g.
    Redis), 1h TTL — NOT a DB table.
  - `POST /auth/reset/confirm` — verify code + set new password.
  - `POST /auth/reset/link` — code-less reset: emailed SIGNED-TOKEN link (stateless,
    no storage) + a fraud-signal notification email so the user can report misuse.
  - Registration uses a 4-digit confirmation code (same cache mechanism).
- **Email subsystem** (new; triggered by auth/account flows, NOT a public endpoint):
  - A new `EmailService` (service, not a controller) that assembles from HTML
    templates in `src/main/resources/` (e.g. `templates/email/`) and sends
    ASYNCHRONOUSLY (`@Async` / mail executor) so the request thread never blocks on
    SMTP; failures logged, not surfaced mid-request.
  - Sender address + SMTP credentials in `application.yml`.
  - Templates: registration-confirm code, reset code, reset link, fraud notification.
- **Single-active-list rule**: application-level. Switching lists deletes the current
  `user_list` (cascades to list_progress/sessions). `GET /me/home` / `GET /enrollments`
  return 0-or-1 in the array (shape unchanged). Optional partial unique index on
  `user_list(user_id) WHERE status='active'` (commented in 01_schema.sql).
- **Sessions API** — spec already written in `prompt-sessions-impl.md`; still pending
  implementation (scaffolding was reverted).
